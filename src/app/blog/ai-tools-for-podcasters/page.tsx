export const metadata = {
  ...
  description: "Podcasting has always been a time-heavy medium. For every hour of audio you publish, there are hours of research, scripting, editing, writing show notes,...",
};
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0f23 0%, #1a1a3e 50%, #0d1b2a 100%)", color: "#fff", fontFamily: "system-ui, sans-serif", padding: "2rem" }}>
      <div style={{ maxWidth: "800px", margin: "0 auto", paddingTop: "2rem" }}>
        ...
        <div style={{ lineHeight: "1.8", fontSize: "1.05rem", color: "#d1d5db" }}>
          ...
          <p>The best podcast interviews happen when the host knows the guest's work well enough to ask questions that the guest has never been asked before. AI makes deep research fast.</p>
...
5. Any potential landmines to avoid (controversies, topics they've said they dislike discussing)`}</pre>
          <p>This prep workflow alone can save an hour per episode and produce sharper questions than a two-hour deep-dive would.</p>
...
[Paste transcript]`}</pre>
          <p>This replaces a task that typically takes 60-90 minutes and produces output that is often better-structured than what most shows publish.</p>
...
My angle / thesis is: [state your specific take on the topic]`}</pre>
          ...
          <p>Most podcasters publish an episode and move on. AI makes it easy to extract full secondary value from every recording. From a single transcript you can generate a LinkedIn post, a Twitter/X thread, a newsletter section, a blog post draft, and YouTube description — in under 20 minutes total.</p>
...
[Paste transcript excerpt or summary]`}</pre>
          ...
          <p>Consistent podcasts win. The shows that grow are the ones that never have a gap. Use <a href="https://notion.so" rel="noopener sponsored" style={{ color: "#00d4ff" }}>Notion</a> with its AI features to maintain a rolling episode pipeline: a database where each episode card has the topic, guest status, recording date, publish date, and repurposing status. Notion AI can help you brainstorm 20 episode ideas in your niche in minutes, which you can batch-process into a 3-month calendar.</p>
...
For each title, rate it on: clarity (does it say what the episode is about?), curiosity (does it make someone want to click?), and searchability (would someone type this to find this content?). 
Then suggest 3 alternatives.`}</pre>
        ...
        <div style={{ display: "flex", gap: "0.75rem", marginTop: "2rem", flexWrap: "wrap" }}>
          {["podcasting", "ai-tools", "content-creation", "workflow"].map(tag => (
